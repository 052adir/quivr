...
> **הנחיה למבקר (GPT):** מסמך עצמאי ומלא: (1) סיכום הפרויקט, (2) סכמת מסד הנתונים המלאה,
> (3) **כל קוד המקור** (Next.js + TypeScript). אין צורך בגישה חיצונית.
>
> **מה לבדוק:** 1) באגים לוגיים וקריאוּת (במיוחד `lib/domain/calc.ts`). 2) ארכיטקטורה — over-engineering?
> 3) אבטחה — RLS פתוח בשלב 1, מה המינימום לפני חשיפה? 4) מודל נתונים ו-GENERATED COLUMNS. 5) שלמות מול אפיון. 6) סדר עדיפויות.
> אנא החזר ממצאים עם הפניה לקובץ/שורה וחומרה (קריטי/בינוני/שיפור).
>
> **הערה:** קיימות שתי גרסאות — אפליקציה עצמאית (קובץ HTML, לא כלול כאן) ומערכת מלאה (Next.js, כל הקוד למטה).
...
| 13 | ייבוא מסמכים | ✅ כל 7 הסוגים פעילים (באפליקציה החיה) | תמחיר עובדים (→עובדים), Z יומי (→יומן, עם מיזוג יום קיים), רווח והפסד (טבלת % מול יעד + המלצות AI), קניות (פוד קוסט), כרטסת ספקים (חריגות מיקוח), מכירות (מובילים/חלשים), מאזן בוחן. הכל בזרימת נתח→תצוגה מקדימה→אישור; ניתוחים נשמרים. במערכת המלאה: מבנה נתונים + אחסון מאובטח (העלאת קבצי PDF/OCR = שלב הבא) |
